...
var regJson = {
	...
	"health" : "/^[0-9]+([.]{1}[0-9]{1})?$/",    // 视力、身高、体重：大于0的正数，支持到小数位后一位
	"score" : "/^[0-9]+([.]{1}[0-9]{1})?$/",    // 等于0或大于0的正数，支持到小数位后一位
	"deduct_point" : "/^-[0-9]+([.]{1}[0-9]{1})?$/",    // 小于0的负数，支持到小数位后一位
	"one_decimal" : "/(^[0-9]+([.][0-9]{1}){0,1}$)|(^-[0-9]+([.][0-9]{1}){0,1}$)/"    // 最多带有一位小数的数字
};
 ...
 */
function javaexVerify(selector, flag) {
	var result = true;
	if (!!selector) {
		// 遍历当前页面所有input元素
		$(selector+" input, "+selector+" textarea, "+selector+" select").each(function() {
			...
			if (!!dataTypeAll) {
				// 防止自定义错误提示后，再次提交
				if ($(this).parent().parent().find(".javaex-field-error-message").length!=0) {
					result = false;
					return result;
				}
	...
	if (flag && result==false) {
		$(".javaex-field-error-message").first().parent().find("[data-type]").focus();
	}
...
function deleteErrorState(obj) {
	obj.parent().parent().find(".javaex-field-error-message").remove();
}
...
function addErrorMsg(obj, errorMsg) {
	if (obj.parent().parent().find(".javaex-field-error-message").length==0) {
		obj.parent().parent().append('<div class="javaex-field-error-message javaexFormErrorTip">'+errorMsg+'</div>');
	}
}
