...
;(function() {
	var javaex = function() {
		...
		var info = {
			...
			deepReplaceKeyName : function(obj, searchStringArr, replaceStringArr) {
				...
				case "array":
					for (let i = 0; i<obj.length; i++) {
						obj[i] = info.deepReplaceKeyName(obj[i], searchStringArr, replaceStringArr);
					}
					break;
				case "object":
					for (let key in obj) {
						...
						obj[newkey] = info.deepReplaceKeyName(obj[key], searchStringArr, replaceStringArr);
						if (key != newkey) {
							delete obj[key];
						}
					}
			...
			/**
			 * 将$("#form").serializeArray()数据格式化成JSON字符串
			 * serializeArray ：$("#form").serializeArray()
			 */
			serializeArrayToJsonString : function(serializeArray) {
				let jsonobj = {};
			...
			/**
			 * 将$("#form").serialize()数据格式化成JSON字符串
			 * serialize ：$("#form").serialize()
			 */
			serializeToJsonString : function(serialize) {
				let formData = decodeURIComponent(serialize);
			...
			getRadioVal : function(name) {
				let val = $(':radio[name="'+name+'"]:checked').val();
			...
			getRadioText : function(name) {
				...
				let text = $(':radio[name="'+name+'"]:checked').next().next(".javaex-fill-text").text();
				if (!text) {
			...
			textarea : function(args) {
				...
				const settings = Object.assign(target, defaults(args));
				const textareaId = settings.id;
				const checklenId = settings.checklenId;
				const maxlength = parseInt(settings.maxlength);
				
				const $textarea = $("#"+textareaId);
				
				if (!!textareaId && maxlength>0) {
					$textarea.prop("maxlength", maxlength);
				}
				
				// 自动高度
				if (settings.autoHeight) {
					autoHeight($textarea[0]);
				...
				function strLenCalc(obj, checklenId) {
					let len = strlen(obj.value);
					$("#"+checklenId).text(len);
			...
			tree : function(args) {
				...
				let html = '';
				if (settings.checkbox && settings.isShowAllCheck) {
						...
						$.each(jsonData, function(i, data) {
							if (!!data.children && data.children.length>0) {
								...
								if (!data.url || settings.isAllowJumpUrl==false) {
									...
									html += '<a href="'+data.url+'" node="'+nodeIndex+'" javaex-menu-id="'+data.id+'" target="_blank">';
								}
								if (icon) {
									if (!!data.icon) {
										flag = true;
				...
				$("#"+id+" li a").on("click", function(e) {
					...
					// 回调函数
					if (settings.isAllowJumpUrl && $(this).attr("href")!="javascript:;") {
			...
			message : function(args) {
				...
				if (type=="submit") {
					if (settings.hasMask) {
						html += '<div class="javaex-opt-mask javaex-mask-black"></div>';
					}
			...
			select : function(args) {
				...
				let dataList = settings.dataList;
				let minWidth = parseInt(settings.minWidth);
				let isSearch = settings.isSearch;
				let hasInputName = settings.hasInputName;
				...
				// 生成样式代码
				let selectHtml = '<div id="span-'+selectId+'" class="javaex-select-box">';
				if (isSearch) {
					if (hasInputName) {
				...
				// 设置宽度
				let width = $select[0].style.width;
				if (!!width) {
					$("#input-"+selectId).css("width", width);
					$("#span-"+selectId).css("width", width);
				} else {
				...
				$("#span-"+selectId).click(function(event) {
					...
					if ($("#"+selectId).prop("disabled")) {
						...
						let ulHeight = $("#ul-"+selectId).height();
						let elHeight = $("#input-"+selectId).height();
						let offsetBottom = $(window).height() - elHeight - ($("#input-"+selectId).offset().top - $(document).scrollTop());
			...
			page : function(args) {
				...
				// 获取默认选中页
				let pageNum = parseInt(settings.pageNum);
				if (pageNum<1 || pageNum>totalPages) {
					pageNum = 1;
				}
				let goPageNum = pageNum;
				...
				// 是否显示跳页
				let isShowJumpPage = settings.isShowJumpPage;
				// 是否显示条数选择控件
				let isShowSelect = settings.isShowSelect;
				// 条数选择控件重新选择后，是否返回第1页
				let isReturnFirst = settings.isReturnFirst;
				// 获取显示位置
				let position = settings.position;
				if (position=="left") {
				...
				// 上一页、下一页文字标识
				let lastPageText = settings.lastPageText;
				let nextPageText = settings.nextPageText;
				...
				function showPage(pageId, type, goPageNum) {
					...
					if (totalPages<=0) {
						...
						let html = '<li class="javaex-last-page"><a>'+lastPageText+'</a></li>';
				...
				function init(pageId, curPageNum) {
					...
					$("#javaex-button-mini-"+pageId).click(function() {
						let goPageNum = $("#javaex-pager-"+pageId).val();
					...
					$("#"+pageId).children("li").click(function() {
						...
						let element = $(this);
						// 当前点击的是第几页
						let pageText = $(this).children("a").text();
						// 当前页
						let pageNum = "";
						// 上一页
						let lastPage = $("#"+pageId).children("li.active").attr("page");
			...
			flow : function(args) {
				...
				function init() {
					// 事先在下方插入加载占位div
					let flowLoadingId = "javaex-flow-loading-" + pageId;
					let obj = document.getElementById(flowLoadingId);
					if (obj==null) {
					...
					$(window).on('scroll',function() {
						let totalheight = $(window).height() + $(window).scrollTop() + bottomH;
						
						if ((info.documentHeight <= totalheight) && (info.isOver!=pageId)) {
							$("#" + flowLoadingId).show();
							info.loadDown(pageId);
						} else {
							$("#" + flowLoadingId).hide();
			...
			swiper : function(args) {
				...
				let $swiper = $("#"+settings.id);
				// 是否自动轮播
				let isAutoPlay = settings.isAutoPlay;
				// 切换效果
				let effect = settings.effect;
				// 开始切换的位置（即从第几张图开始切换），从1开始计
				let startSwiper = parseInt(settings.startSwiper);
				let index = startSwiper - 1;
				
				// 触发图片懒加载
				let $self = $swiper.find(".javaex-swiper-focus-bg li").eq(index).find("img");
				if ($self.length==0) {
				...
				// 自动轮播间隔多少毫秒
				let delay = parseInt(settings.delay);
				if (delay==200) {
					delay = 2000;
				}
				// 定时器
				...
				let swiperBg = $swiper.find(".javaex-swiper-focus-bg");
				let swiperLi = swiperBg.find("li");
				let count = swiperLi.length;
				...
				if (settings.focusCenter) {
					let box = $swiper.find(".javaex-swiper-focus-box");
				...
				function change(show, hide) {
					...
					// 触发图片懒加载
					let $self = $swiper.find(".javaex-swiper-focus-bg li").eq(show).find("img");
					if ($self.length==0) {
			...
			date : function(args) {
				...
				if ($("#"+dateId).parent(".javaex-date-container").length==0) {
					var marginRight = parseInt($("#"+dateId).css("marginRight"));
			...
			uploadImage : function(args) {
				...
				const settings = Object.assign(target, defaults(args));
				let url = settings.url;                      // 请求路径
				let inputId = settings.id;                   // <input type="file" />的id
				let isShowTip = settings.isShowTip;          // 是否显示操作提示
				let uploadText = settings.uploadText;        // 上传图片文字提示
				let maxSize = settings.maxSize;              // 单张图片的大小上限，单位KB，0或空为不限制
				let param = settings.param;
				let header = settings.header;
				let dataType = settings.dataType;            // 图片上传后的返回类型
				let addImg = "data:image/png;base64,";
				
				let $fileContainer = $("#"+inputId).closest(".javaex-file-container");
				let $fileContainerImg = $fileContainer.find("img");
			...
			uploadImages : function(args) {
				...
				let url = settings.url;                      // 请求路径
				let inputId = settings.id;                   // <input type="file" />的id
				let containerId = settings.containerId;      // 图片容器id
				let isShowTip = settings.isShowTip;          // 是否显示操作提示
				let uploadText = settings.uploadText;        // 上传图片文字提示
				let maxSize = settings.maxSize;              // 单张图片的大小上限，单位KB，0或空为不限制
				let param = settings.param;
				let header = settings.header;
				let dataType = settings.dataType;            // 图片上传后的返回类型
				let imgList = settings.imgList;              // 图片回显地址
				let maxNum = parseInt(settings.maxNum);      // 文件上传数量上限
				let imageObj = settings.image;
			...
			edit : function(args) {
				...
				// 首次打开页面，判断此前是否异常关闭
				let localContent = info.getLocalStorage("javaex-edit-content_"+editId);
				...
				if (localContent!=null && localContent!=curContent) {
					let tempTip = '<span class="javaex-edit-tempTip">检测到上次页面未正常关闭。<a href="javascript:;" class="blue" onclick="javaex.recoveryTextEditorDraft(this, \''+editId+'\')">恢复内容</a><a href="javascript:;" class="red ml-10" onclick="javaex.cancelTextEditorDraft(this)">取消</a></span>';
				...
				// 固定工具栏
				if (fixedTop>=0) {
					let toolbarWidth = $("#"+editId+" .javaex-edit-toolbar").width();
					$(window).scroll(function() {
						let distanceFromTop = $("#"+editId).offset().top - document.documentElement.scrollTop;
				...
				$("#"+editId+" .javaex-edit-body-container").bind("mouseup keyup", function() {
					...
					// 把数据存储到本地
					info.setLocalStorage("javaex-edit-content_"+editId, $("#"+editId+" .javaex-edit-body-container").html().replace(/<(script)[\S\s]*?\1>/gi, "").replace(/\r\n/g, "<br/>").replace(/\n/g,"<br/>"));
				...
				if (imageObj==null || imageObj.dataType=="base64") {
					...
					let isShowTip = imageObj.isShowTip;
					if (!isShowTip) {
						isShowTip = false;
					}
					info.uploadImageForEdit({
				...
				$("#"+editId+" .edit-btn-video").bind("click", function() {
					let html = '<div class="javaex-unit clear" style="margin-left: -20px;">\
						<div class="javaex-unit-left tl">\
							<p class="subtitle">链接地址</p></div>\
						<div class="javaex-unit-right">\
							<input type="text" class="javaex-text" id="javaex-edit-video-href_'+editId+'" placeholder="请输入视频绝对地址" autocomplete="off" /></div>\
					</div>\
					<div style="display: grid; grid-column-gap: 20px; grid-template-columns: 1fr 1fr;">\
						<div style="grid-column-end: 1; grid-column-start: 1;">\
							<div class="javaex-unit clear" style="margin-left: -20px;">\
								<div class="javaex-unit-left tl">\
									<p class="subtitle">播放器宽</p></div>\
								<div class="javaex-unit-right">\
									<input type="text" class="javaex-text" id="javaex-edit-video-width_'+editId+'" value="640" placeholder="宽" /></div>\
							</div>\
						</div>\
						<div style="grid-column-end: 2; grid-column-start: 2;">\
							<div class="javaex-unit clear" style="margin-left: -20px;">\
								<div class="javaex-unit-left tl">\
									<p class="subtitle">播放器高</p></div>\
								<div class="javaex-unit-right">\
									<input type="text" class="javaex-text" id="javaex-edit-video-height_'+editId+'" value="400" placeholder="高" /></div>\
							</div>\
						</div>\
					</div>';
					...
					info.alert({
						...
						width: "500px",
						confirm : function() {
							let href = $("#javaex-edit-video-href_"+editId).val();
							let width = $("#javaex-edit-video-width_"+editId).val();
							let height = $("#javaex-edit-video-height_"+editId).val();

							if (!!href && !!width && !!height) {
								let html = '<video controls="controls" width="'+width+'" height="'+height+'" src="'+href+'">您的浏览器不支持 video 标签。</video>';
				...
				$("#"+editId+" .edit-btn-href").bind("click", function() {
					let html = '<div class="javaex-unit clear">\
							<input type="text" class="javaex-text" id="javaex-edit-href_'+editId+'" placeholder="请输入链接地址" autocomplete="off" />\
					</div>\
					<div class="javaex-unit clear" style="margin-bottom: 0;">\
							<input type="text" class="javaex-text" id="javaex-edit-hrefText_'+editId+'" placeholder="请输入链接标题" autocomplete="off" />\
					</div>';
					...
					info.confirm({
						...
						width: "500px",
						confirm : function() {
							let href = $("#javaex-edit-href_"+editId).val();            // 超链接地址
							let hrefText = $("#javaex-edit-hrefText_"+editId).val();    // 超链接显示文本
							...
							if (!!href) {
								...
								let hrefHtml = '<a href="'+href+'" target="_blank">'+hrefText+'</a>';
				...
				// 无序列表
				let isInsertUnorderedList = false;
				...
				$("#"+editId+" .edit-btn-foreColor").bind("click", function() {
					...
					if ($("#"+editId+" .edit-btn-foreColor .javaex-edit-color-menu").length==0) {
						$(this).append(settings.editColor);
					} else {
				...
				$("#"+editId+" .edit-btn-table").bind("click", function() {
					let html = '<div style="display: grid; grid-column-gap: 20px; grid-template-columns: 1fr 1fr;margin-bottom: -20px;">\
						<div style="grid-column-end: 1; grid-column-start: 1;">\
							<div class="javaex-unit clear" style="margin-left: -20px;">\
								<div class="javaex-unit-left tl">\
									<p class="subtitle">表格行数</p></div>\
								<div class="javaex-unit-right">\
									<input type="text" class="javaex-text" id="javaex-edit-table-row-num_'+editId+'" value="2" /></div>\
							</div>\
						</div>\
						<div style="grid-column-end: 2; grid-column-start: 2;">\
							<div class="javaex-unit clear" style="margin-left: -20px;">\
								<div class="javaex-unit-left tl">\
									<p class="subtitle">表格列数</p></div>\
								<div class="javaex-unit-right">\
									<input type="text" class="javaex-text" id="javaex-edit-table-col-num_'+editId+'" value="2" /></div>\
							</div>\
						</div>\
					</div>';
					...
					info.alert({
						...
						width: "360px",
						confirm : function() {
							let rowNum = $("#javaex-edit-table-row-num_"+editId).val();
							let colNum = $("#javaex-edit-table-col-num_"+editId).val();
							let html = '';
				...
				$("#"+editId+" .edit-btn-quote").bind("click", function() {
					info.alert({
						content : '<textarea id="javaex-edit-quote_'+editId+'" wrap="hard" class="javaex-desc" style="height: 160px;"></textarea>',
						title : "请输入引用内容",
						width: "500px",
						confirm : function() {
							let quote = $("#javaex-edit-quote_"+editId).val();
							if (!quote) {
								quote = "<br/>";
							}
							if (!!info.curRange) {
							...
							let quoteHtml = '<div class="javaex-edit-quote"><blockquote>'+quote+'</blockquote></div><br/>';
				...
				function getCurRange() {
					...
					let oEditArea = $("#"+editId+" .javaex-edit-body-container")[0];
					//获取选中区域
					selection = window.document.getSelection();
			...
			// 删除草稿
			deleteTextEditorDraft : function(editId) {
				javaex.deleteLocalStorage("javaex-edit-content_"+editId);
			},
			// 恢复草稿内容
			recoveryTextEditorDraft : function(obj, editId) {
				let localContent = info.getLocalStorage("javaex-edit-content_"+editId);
				$("#"+editId+" .javaex-edit-body-container").html(localContent);
				$("#"+editId+" .javaex-edit-body-container").focus();
				
				$(obj).parent().remove();
			},
			// 取消富文本底部草稿提示
			cancelTextEditorDraft : function(obj) {
				$(obj).parent().remove();
			}
		};
